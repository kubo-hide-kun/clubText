...
![reference](../image/image.001.jpeg)<br>
...
![reference](../image/image.002.jpeg)<br>
特に何も代入されていない状態の変数に値を代入することを**初期化**と言います。
```c
//5行目
printf("%d\n",num);
```
...
これにより配置していた`num`内に格納されている変数が呼び出され、その中に格納されて４位る値を取り出した結果である`5`という数値を表示しています。<br>
このように変数名を記述することで、その変数に格納されている値を取得することを**参照**と言います。
![reference](../image/image.004.jpeg)<br>
...
![reference](../image/image.005.jpeg)<br>
...
```
これは関数の宣言と初期化を **同時** にやっているのです。<br>
これにより今まで２つに分けて記載していた分も一つに求めることが可能になりました。
...
> 510
```
このようにaに新たな値を代入するたびに値は更新されていきます。
